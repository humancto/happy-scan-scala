...
/// Parses lock files (both Coursier format and SBT lock.sbt format)
pub fn parse_lock_file(path: &Path) -> Result<Vec<Dependency>> {
    ...
    // SBT lock.sbt format: "org" % "name" % "version"
    // Generated by sbt-lock plugin with dependencyOverrides
    if fname == "lock.sbt"
        || content.contains("dependencyOverrides")
        || content.contains("sbt-lock")
    {
        return parse_sbt_lock_file(path, &content);
    }
...
/// Parses SBT-format lock files (lock.sbt generated by sbt-lock plugin).
/// These use standard SBT dependency syntax: "org" % "name" % "version"
fn parse_sbt_lock_file(path: &Path, content: &str) -> Result<Vec<Dependency>> {
    let mut deps = Vec::new();
    let re = Regex::new(
        r#"[\"']([^\"']+)[\"']\s*%{1,2}\s*[\"']([^\"']+)[\"']\s*%\s*[\"']([^\"']+)[\"']"#,
    )?;
...
pub fn discover_sbt_files(root: &Path) -> Result<ParsedProject> {
        ...
        .filter_map(|e| e.ok())
    {
        ...
        } else if fname == "lock.sbt" {
            // lock.sbt uses SBT dep syntax ("org" % "name" % "version"),
            // so parse it the same as build.sbt but mark deps as transitive
